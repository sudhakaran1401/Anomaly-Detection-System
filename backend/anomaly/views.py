...
def download_csv(request):
    ...
    uploaded_filename = request.session.get( "uploaded_filename", "dataset.csv", )

    filename, _ = os.path.splitext(uploaded_filename)

    response = HttpResponse(content_type="text/csv")

    response["Content-Disposition"] = f'attachment; filename="{filename}_labelled.csv"'
...
def download_pdf(request):
    metadata = request.session.get("report_metadata", {})

    return PdfService.generate_anomaly_pdf( request, metadata, )
...
def clear_history(request, id=None):
    if id:
        get_object_or_404( DetectionHistory, id=id, ).delete()
    else:
