...
def run_anomaly_pipeline( df, dataset_info, model_name, contamination, scaler_type, dataset_name, ):
    ...
    # A column that is entirely missing cannot be imputed from its mean.
    numeric_df = numeric_df.dropna(axis=1, how="all")

    if numeric_df.empty:
        result_df["result"] = "Normal"
        result_df["anomaly_score"] = 0.0
        add_pca_columns(result_df, np.zeros((len(result_df), 1)))
        return {
            "df": result_df,
            "model_path": None,
        }

    # Any remaining NaN values are replaced with zero after removing
    # completely missing features, so scikit-learn never receives NaN input.
