...
class PdfService:
    @staticmethod
    def generate_anomaly_pdf(request, metadata=None):
        ...
        try:
            ...
            return HttpResponse( f"Unable to read result data: {exc}", status=500, )

        filter_type = request.GET.get("filter")

        data = PdfService._apply_filter( df.to_dict(orient="records"), filter_type, )

        summary = PdfService._build_summary(data)

        # Charts should use the same filtered data as the summary.
        filtered_df = pd.DataFrame(data)

        return PdfService._build_pdf_response( df=filtered_df, summary=summary, metadata=metadata, )

    @staticmethod
    def _apply_filter(data, filter_type):

        if filter_type == "normal":
            return [row for row in data if row.get("result") == "Normal"]

        if filter_type == "anomaly":
            return [row for row in data if row.get("result") == "Anomaly"]
    ...
    def _build_pdf_response(df, summary, metadata):
        ...
        content.append(
            Paragraph( "Smart Anomaly Detection Report", styles["Title"], )
        )
        ...
        content.append( Paragraph( (f"<b>Dataset:</b> {metadata.get('uploaded_filename', 'Unknown')}"), styles["Normal"], ) )

        content.append(Spacer(1, 8))

        content.append( Paragraph( (f"<b>Model:</b> {metadata.get('model_name', 'Unknown')}"), styles["Normal"], ) )

        content.append(Spacer(1, 8))

        content.append( Paragraph( (f"<b>Scaler:</b> {metadata.get('scaler_type', 'Unknown')}"), styles["Normal"], ) )

        content.append(Spacer(1, 8))

        content.append( Paragraph( (f"<b>Contamination:</b> {metadata.get('contamination', 'Unknown')}"), styles["Normal"], ) )
        ...
        summary_data = [
            ...
            [ "Anomaly Percentage", f"{summary['anomaly_percentage']}%", ],
        ]
        ...
        chart_path = ChartService.generate_summary_chart( summary["normal"], summary["anomalies"], )
        ...
        if not df.empty:
            try:
                ...
                if pca_chart_path and os.path.exists(pca_chart_path):
                    content.append( Paragraph( "<b>PCA Visualization</b>", styles["Heading2"], ) )
            ...
            try:
                ...
                if histogram_chart and os.path.exists(histogram_chart):
                    content.append( Paragraph( "<b>Feature Distribution</b>", styles["Heading2"], ) )
            ...
            content.append( Paragraph( "No records are available for the selected filter.", styles["Normal"], ) )
