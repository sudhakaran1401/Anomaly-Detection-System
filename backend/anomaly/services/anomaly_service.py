from anomaly.models import AnomalyResult, DetectionHistory
from anomaly.ml.anomaly import detect_anomalies
from core.ml.dataset_analyzer import DatasetAnalyzer
from anomaly.services.analytic_service import AnalyticsService
from anomaly.services.evaluation_service import EvaluationService
from anomaly.services.explainability_service import ExplainabilityService
from core.services.feature_engineering_services import FeatureEngineeringService
from anomaly.services.report_service import ReportService
import numpy as np


class AnomalyService:

    @staticmethod
    def process_anomalies(
        df,
        filename=None,
        model_name="isolation_forest",
        contamination=0.05,
        scaler_type="standard",
        user=None,
    ):

        df = FeatureEngineeringService.generate_features(df)

        dataset_info = DatasetAnalyzer.analyze(df)

        detection_result = detect_anomalies(
            df=df,
            model_name=model_name,
            contamination=contamination,
            scaler_type=scaler_type,
            dataset_name=filename,
        )

        result_df = detection_result["df"]

        model_path = detection_result["model_path"]

        result_df = AnomalyService._add_anomaly_reasons(result_df)

        total = len(result_df)

        anomalies = len(
            result_df[
                result_df["result"] == "Anomaly"
            ]
        )

        normal = total - anomalies

        predictions = np.where(
            result_df["result"] == "Anomaly",
            1,
            0,
        )

        y_true = None
        if (
            dataset_info["dataset_type"] == "labelled"
            and dataset_info["target_column"] in result_df.columns
        ):
            y_true = result_df[dataset_info["target_column"]].to_numpy()

        evaluation = EvaluationService.evaluate(
            predictions=predictions,
            scores=result_df["anomaly_score"],
            y_true=y_true,
        )

        AnomalyService._save_detection_history(
            filename=filename,
            user=user,
            model_name=model_name,
            scaler_type=scaler_type,
            contamination=contamination,
            total_records=total,
            anomaly_count=anomalies,
            dataset_type=dataset_info["dataset_type"],
            target_column=dataset_info["target_column"],
            model_path=model_path,
            evaluation=evaluation,
        )

        AnomalyResult.objects.create(
            user=user,
            file_name=filename or "dataset.csv",
            total=total,
            normal=normal,
            anomalies=anomalies,
        )

        scatter_data = result_df[
            [
                "pca_x",
                "pca_y",
                "result",
                "anomaly_score",
            ]
        ].to_dict(orient="records")

        score_data = (
            result_df["anomaly_score"]
            .fillna(0)
            .tolist()
        )

        normal_score_data = (
            result_df[
                result_df["result"] == "Normal"
            ]["anomaly_score"]
            .fillna(0)
            .tolist()
        )

        anomaly_score_data = (
            result_df[
                result_df["result"] == "Anomaly"
            ]["anomaly_score"]
            .fillna(0)
            .tolist()
        )

        saved_path = ReportService.save_result_dataframe(result_df)

        return {
            "result_df": result_df,
            "scatter_data": scatter_data,
            "score_data": score_data,
            "normal_score_data": normal_score_data,
            "anomaly_score_data": anomaly_score_data,
            "saved_path": saved_path,
            "total": total,
            "normal": normal,
            "anomalies": anomalies,
            "evaluation": evaluation,
            "report_metadata": {
                "uploaded_filename": filename or "dataset.csv",
                "model_name": model_name,
                "scaler_type": scaler_type,
                "contamination": contamination,
            },
        }

    @staticmethod
    def _add_anomaly_reasons(result_df):

        numeric_columns = result_df.select_dtypes(
            include=["number"]
        ).columns

        column_stats = AnalyticsService.generate_column_stats(
            result_df,
            numeric_columns,
        )

        result_df["reason"] = (
            ExplainabilityService.generate_explanations(
                result_df,
                numeric_columns,
                column_stats,
            )
        )

        return result_df

    @staticmethod
    def _save_detection_history(
        filename,
        user,
        model_name,
        scaler_type,
        contamination,
        total_records,
        anomaly_count,
        dataset_type,
        target_column,
        model_path=None,
        evaluation=None,
    ):

        DetectionHistory.objects.create(
            user=user,
            filename=filename,
            model_name=model_name,
            scaler_type=scaler_type,
            contamination=contamination,
            total_records=total_records,
            anomaly_count=anomaly_count,
            dataset_type=dataset_type,
            target_column=target_column,
            model_path=model_path,
            precision=(
                evaluation.get("precision")
                if evaluation
                else None
            ),
            recall=(
                evaluation.get("recall")
                if evaluation
                else None
            ),
            f1_score=(
                evaluation.get("f1_score")
                if evaluation
                else None
            ),
        )