...
class PdfService:
    ...
    @staticmethod
    def generate_anomaly_pdf(request):
        ...
        metadata = PdfService._get_metadata(request)
    ...
    def _get_metadata(request):
        ...
        return {
            ...
            "model_name": request.session.get( "model_name", "Unknown" ),

            "scaler_type": request.session.get( "scaler_type", "Unknown" ),

            "contamination": request.session.get( "contamination", "N/A" ),

            "uploaded_filename": request.session.get( "uploaded_filename", "Unknown" )
        }
    ...
    def _build_pdf_response(df, summary, metadata):
        ...
        content.append( Paragraph( f"<b>Dataset:</b> " f"{metadata['uploaded_filename']}", styles["Normal"] ) )

        content.append(Spacer(1, 8))

        content.append( Paragraph( f"<b>Model:</b> " f"{metadata['model_name']}", styles["Normal"] ) )

        content.append(Spacer(1, 8))

        content.append( Paragraph( f"<b>Scaler:</b> " f"{metadata['scaler_type']}", styles["Normal"] ) )

        content.append(Spacer(1, 8))

        content.append( Paragraph( f"<b>Contamination:</b> " f"{metadata['contamination']}", styles["Normal"] ) )
